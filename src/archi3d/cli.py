...
def batch_create(
    run_id: Optional[str] = typer.Option(None, "--run-id", help="Run identifier (autogenerated if omitted)"),
    algos: Optional[str] = typer.Option(None, "--algos", help="Comma-separated algorithm keys"),
    image_policy: str = typer.Option("use_up_to_6", "--image-policy", help="Image selection policy"),
    limit: Optional[int] = typer.Option(None, "--limit", help="Maximum number of items to process"),
    include: Optional[str] = typer.Option(None, "--include", help="Include filter (substring match on product_id/variant/product_name)"),
    exclude: Optional[str] = typer.Option(None, "--exclude", help="Exclude filter (substring match)"),
