...
def calculate_stats(data_list):
    ...
    for d in data_list:
        algo = d['algorithm']
        if algo not in algos: algos[algo] = {'fscore': [], 'vfscore': [], 'time': []}
        algos[algo]['fscore'].append(d['fscore'])
        algos[algo]['vfscore'].append(d['vfscore'])
        if d['time'] is not None: algos[algo]['time'].append(d['time'])
    ...
    for metric in ['fscore', 'vfscore']:
        for a1, a2 in combinations(algo_names, 2):
            ...
            if a1 not in stats['pairwise'][metric]: stats['pairwise'][metric][a1] = {}
            if a2 not in stats['pairwise'][metric]: stats['pairwise'][metric][a2] = {}
...
def generate_html():
    ...
    with open(CSV_PATH, 'r', encoding='utf-8-sig') as f:
        ...
        for row in reader:
            row = {k.strip(): v for k, v in row.items()}
            if not row.get('fscore') or row['fscore'].strip() == '': continue
            if not row.get('vfscore_overall') or row['vfscore_overall'].strip() == '': continue
            try:
                fscore = float(row['fscore'])
                vfscore_overall = float(row['vfscore_overall'])
                if vfscore_overall == 0: continue
                vfscore = vfscore_overall / 100.0
                exec_time = None
                if row.get('generation_duration_s'):
                    try: exec_time = float(row['generation_duration_s'])
                    except: pass
    ...
    print(f"DEBUG: Items: {len(data)}, Runs: {len(run_ids)}")
    ...
    <div class="tab-content">
        ...
        <div class="tab-pane fade" id="stats-pane">
            <div class="help-box">
                ...
                <ul class="mb-0 mt-1 ps-3">
                    <li><strong>Statistiche Descrittive:</strong> Mostra la media e la deviazione standard (±) per ogni metrica. <em>Nota: Per i tempi di esecuzione, gli outlier sono stati rimossi per fornire una stima più stabile.</em></li>
                    <li><strong>Test di Mann-Whitney U:</strong> Verifica se la differenza tra due algoritmi è significativa.</li>
                    <li><strong>Interpretazione p-value:</strong>
                        <ul class="mb-0">
                            <li><span class="badge bg-success text-white">Verde (p < 0.001)</span>: Differenza <strong>molto significativa</strong>. È estremamente improbabile che sia dovuta al caso.</li>
                            <li><span class="badge bg-warning text-dark">Giallo (p < 0.05)</span>: Differenza <strong>significativa</strong>. C'è una buona probabilità che un algoritmo sia effettivamente diverso dall'altro.</li>
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="card">
                        <div class="card-header">Significatività Pairwise (Mann-Whitney U) - F-Score</div>
                        <div class="card-body">
                ...
                <div class="col-md-6">
                    <div class="card">
                        <div class="card-header">Significatività Pairwise (Mann-Whitney U) - VF-Score</div>
                        <div class="card-body">
        ...
        <div class="tab-pane fade" id="sparse-pane">
            <div class="help-box">
                ...
                <ul class="mb-0 mt-1 ps-3">
                    ...
                    <li><strong>Leaderboard:</strong> Classifica gli algoritmi basandosi sulla distanza dal punto ideale (1.0, 1.0). Minore è la distanza, migliore è il compromesso tra qualità geometrica (F-Score) e visiva (VF-Score).</li>
                </ul>
            </div>
            <div class="row">
